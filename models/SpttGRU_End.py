...
import time
from tools.GradientAccumulator_Endpoint_GRU import GradientAccumulatorEndpointGRU
...
class CustomGRUCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, krank, slide_window_nums, device):
        ...
        self.accumulator = GradientAccumulatorEndpointGRU()
    ...
    def forward(self, inputs, hx, valid_mask, is_endpoint):
...
class GRUCellFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, inputs, hidden_state, w_ih, w_hh, b_ih, b_hh, valid_mask, is_endpoint, cell_ref):
    ...
    def backward(ctx, grad_hy):
        ...
        b_ih = torch.cat((b_ir, b_iz, b_in))
        b_hh = torch.cat((b_hr, b_hz, b_hn))
        ...
        # print(f"is_endpoint: {is_endpoint}")
        if is_endpoint and cell.flag:
            # print("start accumulating gradients at endpoint...")
            # 计算更新门的梯度
            ...
            d_input = torch.zeros_like(inputs)
            d_hidden_state = torch.zeros_like(hidden_state)
            grad_w_ih = torch.zeros_like(w_ih)
            grad_w_hh = torch.zeros_like(w_hh)
            grad_b_ih = torch.zeros_like(b_ih)
            grad_b_hh = torch.zeros_like(b_hh)
        
        return d_input, d_hidden_state, grad_w_ih, grad_w_hh, grad_b_ih, grad_b_hh, None, None, None