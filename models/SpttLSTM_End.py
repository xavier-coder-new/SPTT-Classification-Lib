...
class CustomLSTMCell(nn.Module):
    def __init__(
        ...
        self.accumulator = GradientAccumulatorEndpointLSTM()
    ...
    def forward(self, inputs, hx, cx, valid_mask, is_endpoint):
...
class LSTMCellFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, inputs, hx, cx, w_ih, w_hh, b_ih, b_hh, valid_mask, is_endpoint, cell_ref):
    ...
    def backward(ctx, grad_hy, grad_cy):
        ...
        if is_endpoint and cell.flag:
            ...
            if input_cat is None:
                raise ValueError("Endpoint input_cat is None")
            ...
            with torch.no_grad():
                ...
                for i in range(1, num_blocks + 1):
                    ...
                    Sigma_ih_product = torch.sum(
                        ((delta_block @ Delta_matrix_ih) * (activation_input @ X_matrix_ih)) / block_len,
                        dim=0,
                    )
                    Sigma_ih = i_factor * Sigma_ih + update_factor * Sigma_ih_product

                    Sigma_hh_product = torch.sum(
                        ((delta_block @ Delta_matrix_hh) * (activation_hx @ X_matrix_hh)) / block_len,
                        dim=0,
                    )
